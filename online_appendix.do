        ...
        qui:levelsof `j', local(jlevel)
            ...
            foreach ii of local ilevel {
                foreach jj of local jlevel {
                    ...
                    matrix bend_d = nullmat(bend_r)\[`ii',`jj',r(N),r(sum_w),r(mean),r(p50),r(sd)]
                    qui:sum hprod_wkend [w=fwt] if `i' == `ii' & `j' == `jj' & survey == 20 , d
                    matrix bend_r = nullmat(bend_d)\[`ii',`jj',r(N),r(sum_w),r(mean),r(p50),r(sd)]
                    qui:sum hprod_wkday [w=fwt] if `i' == `ii' & `j' == `jj' & survey <  20 , d
                    matrix bday_d = nullmat(bday_r)\[`ii',`jj',r(N),r(sum_w),r(mean),r(p50),r(sd)]
                    qui:sum hprod_wkday [w=fwt] if `i' == `ii' & `j' == `jj' & survey == 20 , d
                    matrix bday_r = nullmat(bday_d)\[`ii',`jj',r(N),r(sum_w),r(mean),r(p50),r(sd)]
...
matrix colname bday_d = vi vj n wn mean median sd
	frame toplot: {
		...
		global opt1 mlcolor(navy*1.2) color(%20)  
		save "/Users/aashimasinha/Documents/GitHub/uslimtipqm/procdata/qtable_`year'", replace
	}
}
